...
class TestKafkaCoordinatorIntegration(KafkaIntegrationTestCase):
    ...
    def test_failed_group_join(self):
        ...
        self.assertEqual(coordinator.coordinator_id, None)
        coordinator.coordinator_id = 15
        coordinator._coordinator_dead_fut = self.loop.create_future()
    ...
    def test_get_offsets(self):
        ...
        await producer.send('topic1', b'first msg', partition=0)
        await producer.send('topic1', b'second msg', partition=1)
        await producer.send('topic1', b'third msg', partition=1)
        await producer.stop()
    ...
    def test_coordinator_close(self):
        ...
        close_task = asyncio.ensure_future(
            coordinator.close(), loop=self.loop)
    ...
    def test_coordinator_ensure_coordinator_known(self):
        ...
        async def _do_coordinator_lookup():
    ...
    def test_coordinator__do_heartbeat(self):
        ...
        coordinator._rejoin_needed_fut = self.loop.create_future()
    ...
    def test_coordinator__coordination_routine(self):
        ...
        def start_coordination():
            coordinator._coordination_task = task = asyncio.ensure_future(
        ...
        done_fut = self.loop.create_future()
